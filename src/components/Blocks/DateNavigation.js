...
const DateNavigation = ({ currentDate, changeDate, changeMonth, changeYear }) => {
  const commonClasses = "ml-4 outline-none transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110";
  ...
  return (
    <div className="flex items-center justify-start mb-4">
      ...
      <ArrowButton direction={1} changeDate={changeDate} />
      <select className={commonClasses} value={currentDate.getMonth()} onChange={changeMonth}>
      ...
      <select className={commonClasses} value={currentDate.getFullYear()} onChange={changeYear}>
        {Array.from({ length: 12 }, (_, i) => (
